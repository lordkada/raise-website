    ...
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${isScrolled ? 'bg-raise-black/80 backdrop-blur-md border-b border-white/10' : 'bg-transparent'}`}>
      <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-8">
          ...
          <button onClick={openModal} className="bg-raise-neon text-raise-black px-6 py-2.5 rounded-full font-bold text-sm hover:bg-raise-yellow hover:scale-105 transition-all duration-300 shadow-[0_0_10px_rgba(204,255,0,0.3)]">
            Prenota Demo
          </button>
        </div>
      ...
      {mobileMenuOpen && (
        <div className="md:hidden bg-raise-dark border-b border-white/10 absolute w-full px-6 py-4 flex flex-col gap-4 shadow-xl">
          ...
          <button onClick={() => {
            setMobileMenuOpen(false);
            openModal();
          }} className="bg-raise-neon text-raise-black px-6 py-3 rounded-lg font-bold text-center mt-2">
            Prenota Demo
          </button>
        </div>
      )}
